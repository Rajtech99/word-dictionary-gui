...
def info():
    ...
    if 0 < len(input_text) < 2:
        messagebox.showerror(title="Error", message=f"{input_text} is not a meaningful word")
    ...
    # Check the entry box empty or not
        ...
        if code == 200:
            ...
            Label(win, text=meaning, font=('calibre', 10, 'normal'), bg="gray90", wraplength=330, fg="blue").place(x=40, y=335, height=100, width=330)
            ...
            if len(synonyms) != 0:
                ...
                Label(win, text=synonyms, font=('calibre', 10, 'normal'), bg="gray90", wraplength=330,
                          fg="red").place(x=40, y=550, height=80, width=330)
            else:
                Label(win, text="Synonyms not available", font=('calibre', 10, 'normal'), bg="gray90", wraplength=330,
                      fg="red").place(x=40, y=550, height=60, width=330)
        else:
            messagebox.showerror(title="Error occurred", message="This word meaning is not define in US English")
    else:
...
def mainWindow():
    ...
    # heading for meaning, audio and synonyms
